"""Diary — read views over experiments.jsonl (metric-agnostic).

Promoted from playground-s6e5/src/diary.py (2026-05-25) and generalized to the
metric-neutral schema written by the shared :mod:`observer`. Renders the
asymmetric interface:

- Claude reads ``experiments.jsonl`` programmatically.
- Human reads markdown rendered by :func:`render_all` into ``docs/diary.md``.
- Both can use the CLI.

Wire it into a project with a tiny shim (so ``python -m src.diary ...`` keeps
working), e.g. ``src/diary.py``::

    from kaggle_playground_utils import diary as _diary
    from .config import PROJECT_NAME  # noqa: F401 — configure() already called
    import sys
    if __name__ == "__main__":
        sys.exit(_diary.cli(sys.argv[1:]))
...
from .observer import _cfg, _load_jsonl, add_note


def _metric_label() -> str:
    return _cfg().metric.name.upper()


def _title() -> str:
    return _cfg().root.name
...
def timeline() -> str:
    rows = _entries()
    if not rows:
        return "(no experiments yet — run one via observer.Experiment)\n"
    label = _metric_label()
    lines = []
    lines.append(
    ...
    return f"metric: {label} ({'↑ better' if _cfg().metric.greater_is_better else '↓ better'})\n" + "\n".join(lines) + "\n"
...
def compare(v_a: str, v_b: str) -> str:
    ...
    if pred_b is not None and act_b is not None:
        ...
        lines.append(f"  Predicted improvement: {pred_b:+.5f}")
        lines.append(f"  Actual improvement:    {act_b:+.5f}")
...
def regressions() -> str:
    ...
    lines = ["# silent regressions (holdout worse than parent)"]
    for e in out:
        lines.append(
            f"  {e['version']} (parent={e['parent']}): holdout {e['parent_holdout_score']:.5f} → "
            f"{e['holdout_score']:.5f} (Δimprove={e['actual_delta']:+.5f})  hyp={e['hypothesis']!r}"
        )
    return "\n".join(lines) + "\n"
...
def _md_per_version(e: dict) -> str:
    ...
    lines.append("")
    lines.append(f"- **Predicted improvement**: `{pred:+.5f}`" if pred is not None else "- **Predicted improvement**: ?")
    lines.append(f"- **Actual improvement**: `{act:+.5f}`" if act is not None else "- **Actual improvement**: (no parent or no result yet)")
    lines.append(f"- **Match**: {match_str}")
    lines.append(f"- **Confidence stated**: {e.get('confidence', '?')}")
    lines.append("")
    lines.append("## Metrics")
    lines.append(f"- **OOF {label}**: `{e.get('oof_score_mean'):.5f}`" if e.get("oof_score_mean") is not None else f"- OOF {label}: ?")
    folds = e.get("oof_score_per_fold") or []
    if folds:
        lines.append(f"- **Per-fold {label}**: " + ", ".join(f"`{x:.5f}`" for x in folds))
    lines.append(f"- **Holdout {label}**: `{e.get('holdout_score'):.5f}`" if e.get("holdout_score") is not None else f"- Holdout {label}: ?")
...
def _md_index() -> str:
    label = _metric_label()
    arrow = "↑ better" if _cfg().metric.greater_is_better else "↓ better"
    rows = _entries()
    lines = [
        f"# {_title()} — Experiment Diary",
        "",
        f"Metric: **{label}** ({arrow}). Source of truth: `experiments.jsonl` "
        f"(git-tracked, append-only). Auto-regenerated by `python -m src.diary render`.",
        "",
        "## Timeline",
        "",
        f"| ver | parent | predicted Δ | actual Δ | holdout {label} | OOF {label} | gap | hypothesis | flags |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for e in rows:
        ver = e.get("version", "?")
...
def cli(argv: list[str]) -> int:
    ...
    if cmd == "timeline":
        ...
        render_all()
        print(f"rendered {_cfg().docs / 'diary.md'} and per-version pages.")
    else:
